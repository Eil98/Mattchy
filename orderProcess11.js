$(document).ready(function () {
    ...
    $(".trash-red").click(function () {
        ...
        if(data.orders[$(this).attr("id")].status ==1){
            ...
            for (let j = 0; j < data.orders[$(this).attr("id")].content.length; j++) {           //Gestione quantitá dopo evasione ordine
                for (let x = 0; x < objs.length; x++) {
                    if (data.orders[$(this).attr("id")].content[j].name == objs[x].name) {
                        ...
                        newqta=parseInt(objs[x].wardrobe[mappingSize(data.orders[$(this).attr("id")].content[j].selected_size)].number) + parseInt(data.orders[$(this).attr("id")].content[j].quantity);

                        console.log("newqta:  "+newqta);
                        objs[x].wardrobe[mappingSize(data.orders[$(this).attr("id")].content[j].selected_size)].number = newqta.toString();
                    }
                }
            }
